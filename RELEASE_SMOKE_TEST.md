...
- ระหว่างอัดให้สลับไปใช้งานแอปเป้าหมายอย่างน้อย 30 วินาที แล้วกลับเข้า ZoomCut; UI ต้องตอบสนองและปุ่ม `หยุดอัด` ต้องกดได้ทันที
- ทดสอบหยุดแยกสามรอบผ่าน HUD, menu bar `Stop Recording` และ `Control-Command-S`; ทุกทางต้อง finalize วิดีโอและโหลดกลับเข้า editor
- ระหว่างอัด Diagnostics/status ต้องรายงาน click hook worker โดย HTTP API และหน้าต่าง ZoomCut ยังตอบสนองตามปกติ
- รอบที่ติดตั้งจาก DMG ต้องเห็น `hookRunning: true` และหลังหยุดไฟล์ `.clicks.json` ต้องมีจำนวนคลิกมากกว่า 0; การทดสอบเฉพาะ `npm start` ยืนยันสิทธิ์ Input Monitoring ของแอปที่เซ็นแล้วไม่ได้
